...
description: "CWV thresholds haven't changed but measurement has. The 2026 audit checklist with INP replacing FID, field data strategies, and real optimizations."
date: "2026-02-10"
author: "Alex Mayhew"
...
On data-heavy SaaS dashboards, the median INP tends to land around 350-500ms. Google's "good" threshold is 200ms.
...
```typescript
// Feature-detect scheduler.yield (not supported in Safari as of March 2026)
...
```typescript
// Long Animation Frames API (LoAF) ... the modern replacement for longtask observers
// Shipped in Chrome 123+, provides richer attribution than longtask
if (PerformanceObserver.supportedEntryTypes.includes("long-animation-frame")) {
	...
	// Fallback to longtask API for non-Chromium browsers
	new PerformanceObserver((list) => {
		for (const entry of list.getEntries()) {
			if (entry.duration > 50) {
