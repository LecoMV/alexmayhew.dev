...
description: "Seven REST API design mistakes that show up in most production APIs. Fix them now, or pay the price when you have 1000 integrators instead of 10."
date: "2026-01-28"
author: "Alex Mayhew"
...
Across APIs from startups to enterprises, these seven mistakes appear in most of them. The earlier you fix them, the cheaper the fix.
...
Cursor queries also play better with the indexes you've already built for [database query optimization](/blog/database-query-optimization)... the ordering column becomes a natural seek target instead of a LIMIT/OFFSET scan.
...
Idempotency is the synchronous cousin of [event-driven architecture](/blog/event-driven-architecture-saas)... once you're running async workflows, every consumer needs idempotent handlers for the same reason HTTP retries need them.
