...
38% is high because Airbnb moves fast. Their JavaScript codebase evolves rapidly, which increases the odds of type-related errors slipping through. This compounds with every architectural choice... the patterns documented in [modern frontend architecture](/blog/modern-frontend-architecture) all assume type-safe contracts between layers.
...
These features transform code navigation. New developers understand the codebase faster because the tools tell them what exists and how to use it. The effect extends to [component API design](/blog/component-api-design)... typed props force you to think about what your component actually needs before you write it.
...
Use both. Tests verify logic. Types verify contracts. For components that straddle client and server... like those covered in [testing React Server Components](/blog/testing-react-server-components)... types are the only reliable way to enforce the serialization boundary.
