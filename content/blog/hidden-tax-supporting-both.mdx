...
description: 'Every SaaS team says "we''ll support both" databases, auth, or deploy targets. The hidden cost: 2x testing, 2x docs, and bugs in one configuration.'
date: "2026-02-28"
author: "Alex Mayhew"
...
async function getUser(id: string): Promise<User> {
	if (config.database === "postgresql") {
		...
		return db.query(`SELECT *, preferences->>'theme' as theme FROM users WHERE id = $1`, [id]);
	} else {
		// MongoDB: dot notation for nested fields
		return db
			.collection("users")
			.findOne({ _id: new ObjectId(id) }, { projection: { "preferences.theme": 1 } });
	}
}
```
...
| PostgreSQL and MongoDB | Pick one based on workload             | MongoDB excels at document-heavy workloads; PostgreSQL covers relational + JSONB. Supporting both is rarely worth the adapter complexity. |
