...
describe("PromptRegression", () => {
  ...
  it("new prompt does not regress on established test cases", async () => {
    ...
    for (const testCase of testCases) {
      ...
      currentScores.push(
        scoreOutput(currentResult, testCase.expectedProperties)
      );
      previousScores.push(
        scoreOutput(previousResult, testCase.expectedProperties)
      );
    }
    ...
    for (let i = 0; i < testCases.length; i++) {
      expect(currentScores[i]).toBeGreaterThanOrEqual(
        previousScores[i] * 0.8
      );
    }
  });
});
...
describe("ContentModerationPipeline", () => {
  ...
  it("never approves content containing blocked patterns", () => {
    fc.assert(
      fc.property(
        fc.stringOf(fc.constantFrom(...BLOCKED_PHRASES)),
        (maliciousContent) => {
