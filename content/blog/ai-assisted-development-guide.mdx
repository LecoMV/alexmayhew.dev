...
**pgvector (< 1M vectors)** - If you are already running PostgreSQL, start here. Single database, ACID transactions, familiar tooling. HNSW indexes consume roughly 1.5GB per million 1536-dimensional vectors.

**Qdrant (1-10M vectors)** - Best balance of performance, features, and operational complexity. Native metadata filtering, self-hostable, binary quantization reduces memory 4-32x.

**Pinecone (> 10M vectors or managed preference)** - Zero ops overhead, linear cost scaling. At 100 million queries/month, expect $9,600 for reads alone.
...
**Style consistency** - Configure rules once, AI enforces them across every PR. Human reviewers get tired and develop blind spots. AI applies the same standard to the first PR of Monday morning and the last PR of Friday evening.

**Common bug patterns** - Off-by-one errors, null references, race conditions, resource leaks. These bugs are well-documented in training data. AI has seen them thousands of times.

**Documentation gaps** - Functions without JSDoc, missing parameter descriptions, undocumented return types. AI notices what is missing as effectively as what is wrong.

### What LLMs Miss

**Business logic correctness** - AI cannot evaluate whether your code does what the business needs. It validates code against patterns—not against intent.

**Security edge cases** - AI recognizes common vulnerabilities. It misses domain-specific threats like missing authorization checks in financial transfer logic.

**Architectural fit** - AI trained on general code patterns has no visibility into your specific patterns. It cannot enforce your hexagonal architecture or domain-driven design boundaries.

**Performance at scale** - AI reviews code in isolation. It cannot assess whether your five sequential database calls will timeout under production load.

### The Hybrid Workflow

The solution is AI then humans, with clear boundaries.

**Stage 1: AI pre-review** - Before a PR reaches human reviewers, AI scans for style violations, common bug patterns, missing documentation, dependency vulnerabilities.

**Stage 2: Human review** - Humans focus on business logic, architecture fit, domain-specific security, performance at scale, test quality. The mechanical checks are already resolved.

**Stage 3: Merge gate** - All AI issues resolved or explicitly overridden, human approval recorded, tests pass, security scan passes.
