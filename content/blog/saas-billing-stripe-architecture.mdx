...
const PLAN_ENTITLEMENTS: Record<string, Entitlements> = {
  ...
  'price_enterprise_monthly': {
    ...
    maxProjects: -1,
    features: ['basic_analytics', 'advanced_analytics', 'priority_support', 'api_access', 'sso', 'audit_log', 'custom_branding'],
    apiRateLimit: 10000,
    storageGB: 500,
  },
};
...
async function reportDailyUsage(tenantId: string) {
  ...
  if (storageItem) {
    await reportUsage(storageItem.id, Math.ceil(usage.storage / (1024 * 1024 * 1024)), Math.floor(Date.now() / 1000));
  }
}
```
